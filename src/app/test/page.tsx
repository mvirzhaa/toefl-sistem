...
import { toPng } from 'html-to-image'; // <--- LIBRARY BARU (Anti Error Lab)
...
export default function TestPage() {
  ...
  const finishTest = async () => {
    ...
    let tempScore = 0;
    questions.forEach(q => {
      if (answers[q.id] === q.correctAnswer) tempScore += 1;
    });
    
    // Hitung Detail Per Bagian
    const listeningScore = questions.filter(q => q.category === 'LISTENING' && answers[q.id] === q.correctAnswer).length;
    const structureScore = questions.filter(q => q.category === 'STRUCTURE' && answers[q.id] === q.correctAnswer).length;
    const readingScore = questions.filter(q => q.category === 'READING' && answers[q.id] === q.correctAnswer).length;
    
    // Hitung Prediksi Skor (Rumus Sederhana)
    ...
    try {
      ...
      console.log("✅ Nilai tersimpan di database.");

      // 3. KIRIM EMAIL OTOMATIS (LOGIKA BARU)
      if (uData.email) {
        await fetch('/api/test/email', {
          ...
          body: JSON.stringify({
            name: uData.name,
            email: uData.email,
            score: predictionScore, // Kirim skor konversi
            details: {
        ...
        console.log("📧 Email hasil telah dikirim!");
      }
  ...
  // 4. LOGIKA DOWNLOAD PDF BARU (Menggunakan html-to-image)
  const downloadPDF = async () => {
    if (!certificateRef.current) return;
    
    try {
      // Ubah HTML jadi Gambar PNG dulu
  ...
  if (isFinished) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 py-10 px-4 flex flex-col items-center justify-center font-sans">
        ...
        {/* AREA SERTIFIKAT (Background Putih dipaksa agar stabil saat print) */}
        <div 
          ref={certificateRef} 
          style={{ backgroundColor: '#ffffff' }} 
          className="p-10 w-full max-w-3xl shadow-2xl rounded-xl border-4 border-double border-slate-200 relative overflow-hidden text-slate-900"
        >
        ...
        <div className="mt-8 flex flex-col md:flex-row gap-4 w-full max-w-3xl">
  ...
  // --- TAMPILAN SOAL UJIAN (Desain Asli) ---
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 scroll-smooth pb-32">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-8"><p className="text-xl lg:text-2xl text-slate-800 leading-relaxed font-medium">{currentQ.questionText}</p></div>
        ...
        <aside className={`absolute lg:relative top-0 right-0 h-full w-80 bg-white border-l border-slate-200 z-10 transform transition-transform duration-300 ease-in-out flex flex-col ${showSidebar ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
          <div className="p-6 bg-slate-50 border-b border-slate-100"><h3 className="font-bold text-slate-800 flex items-center gap-2"><LayoutGrid className="w-5 h-5 text-blue-500" /> Navigasi Soal</h3><p className="text-xs text-slate-500 mt-1">Klik nomor untuk lompat ke soal.</p></div>
